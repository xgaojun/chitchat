...
func (user *User) CreateThread(topic string) (conv Thread, err error) {
	...
	//new code for mysql
	err = Db.QueryRow("SELECT id, uuid, topic, user_id from threads where last_insert_id()").
...
func (user *User) CreatePost(conv Thread, body string) (post Post, err error) {
	...
	//new code for mysql
	err = Db.QueryRow("SELECT id, uuid, body, user_id, thread_id from posts where last_insert_id()").
		Scan(&post.Id, &post.Uuid, &post.Body, &post.UserId, &post.ThreadId)
	if err != nil {
		return
	}
