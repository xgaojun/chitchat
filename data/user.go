...
func (user *User) CreateSession() (session Session, err error) {
	...
	//new code for mysql
	err = Db.QueryRow("SELECT id, uuid, email, user_id from sessions where last_insert_id()").
...
func (user *User) Create() (err error) {
	...
	//new code for mysql
	err = Db.QueryRow("SELECT id, uuid from users where last_insert_id()").
...
func (user *User) Update() (err error) {
	statement := "update users set name = ?, email = ? where id = $1"
	stmt, err := Db.Prepare(statement)
	if err != nil {
		return
	}
...
func UserByEmail(email string) (user User, err error) {
	user = User{}
	err = Db.QueryRow("SELECT id, uuid, name, email, password FROM users WHERE email = ?", email).
